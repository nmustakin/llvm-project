...
struct AAKernelInfoImpl : AAKernelInfo {
  AAKernelInfoImpl(const IRPosition &IRP, Attributor &A) : AAKernelInfo(IRP, A) {}
  ...
  /// Create an abstract attribute biew for the position \p IRP.
  static AAKernelInfoImpl &createForPosition(const IRPosition &IRP, Attributor &A);
  ...
  /// This function should return true if the type of the \p AA is AAKernelInfoImpl
  static bool classof(const AbstractAttribute *AA) {
    return (AA->getIdAddr() == &ID);
  }
...
struct AAKernelInfoCallSite : AAKernelInfoImpl {
  ...
  void initialize(Attributor &A) override {
    ...
    // will use an AAKernelInfoImpl object on the callee to gather information and
